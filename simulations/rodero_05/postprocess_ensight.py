...
def extract_u_p1(h5file, mesh, p1_to_p2, time_str):
    """Extract P2 displacement and interpolate to P1 corner nodes.
    p1_to_p2 is precomputed once outside this function (same for every timestep)."""
...
def extract_u_p1_h5py(h5file, p1_to_p2, time_str):
    path = f'mechanics/u/{time_str}'
    with h5py.File(h5file, 'r') as f:
        grp = f[path]
        vector = grp['vector_0'][:]
    p2_vals = vector.reshape(-1, 3)
    return p2_vals[p1_to_p2]
...
def build_p1_to_p2_map(mesh):
    ...
    p1_to_p2 = np.zeros(n_p1_nodes, dtype=np.int64)  # explicit int dtype
    for cell in dolfin.cells(mesh):
        p1_corners = p1_topo[cell.index()]
        p2_dofs = dm.cell_dofs(cell.index())[::3] // 3
        for j in range(4):
            p1_to_p2[p1_corners[j]] = int(p2_dofs[j])  # force scalar int
    return p1_to_p2
...
def convert(h5file, outdir, tv_file=None):
    ...
    rprint('Loading EP mesh for interpolation...')
    ...
    for i in my_timestep_indices:
        ...
        if t_str in mech_time_strs:
            u_vals = extract_u_p1_h5py(h5file,p1_to_p2, t_str)
        else:
    ...
    for field_name in ep_fields:
        ...
        pattern = f'{var_name}.****'
        rprint(f'Processing {var_name} (EP->mechanics)...')
        for i in my_timestep_indices:
            t_str = all_time_strs[i]
            out_path = os.path.join(outdir, f'{var_name}.{i+1:04d}')
    ...
    # Material labels — trivial per-timestep cost, but keep it split too for consistency
...
def convert(h5file, outdir, tv_file=None):
    ...
    p1_to_p2 = build_p1_to_p2_map(mesh)
    ...
    for field_name in ep_fields:
        # Native resolution — separate output dir/prefix to avoid filename collision
        ...
        # Interpolated onto mechanics mesh — different filename prefix so it
        # doesn't collide with the native version above
    ...
    # ── Material labels (mechanics mesh only, unchanged) ──────────────────────
    if tv_file and os.path.exists(tv_file):
        coarse_tv = np.load(tv_file)
        var_name = 'material_tv'
        pattern = f'{var_name}.****'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results', default='results/biv_coarse_run_output/results.h5')
    parser.add_argument('--out', default='results/ensight')
